...
function DeliveryAddress() {
  ...
  useEffect(() => {
    const fetchDeliveryAddress = async () => {
      const response = await fetch(`http://localhost:5000/patient/delivery/delivery-address`, {
        method: "GET",
        headers: {
  ...
  return (
    <>
        {addresses && (
          <Select value={selectedAddress} onChange={handleCategoryChange} style={{ width: 200 }}>
            <Option value="" disabled hidden>Choose Delivery Address</Option>
            {addresses.map((address, index) => (
              <Option key={index} value={address.street_address+', '+address.city+', '+ address.governate}>
      ...
      <Modal title="Delivery Address" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
        <label>
            Street Address:
            <input
              type="text"
